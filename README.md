...
### ノイズ: BA（background activity）が誤検知の律速

静止シーンでの誤検知を調べたところ、ノイズイベントは
**時間的にも空間的にも散らばった孤立バースト**だった（検知間隔 min 24 ms /
p50 192 ms、全画面に分布、同一セルの再発なし）。
つまり照明フリッカでもホットピクセルでもない、典型的な BA ノイズ。
...
### 未了: 誤検知率の定量化にはシーン制御が要る

パラメータ掃引を試みたが、**同一設定の 3 秒 x 6 回でイベントレートが
0.005〜0.019 Mev/s とばらつき**、掃引結果を解釈できなかった。
誤検知率を測るには:

- 一様に照らした無地の面（白壁・紙）に向ける。レンズキャップは
  低照度でフォトレセプタが遅く・ノイジーになるので不適
- 照明は DC（フリッカのないもの）
- 撮影範囲内で誰も動かない状態を維持

この条件を作れば「感度 ↔ 誤検知」のカーブを引いて動作点を決められる。

## 未了

- [ ] 動き検知の誤検知率カーブ（要: 制御されたシーン）
