...
**ROI は設定を受け付けるが、効いていない。** `set_mode(Mode.ROI)` → `set_window(Window(200,150,120,100))`
→ `enable(True)` としても、ROI 外のイベントが出続ける（x が 0..639 に達する）。
ストリーム開始前・開始後のどちらで設定しても同じだった。
一度だけ「箱の中 73.9%」という結果が出たが再現せず、シーン内の動きが偶然その領域に
集中しただけと考えられる。**検証時にシーンが静止していて母数が数百イベントしか取れておらず、
決定的な確認ができていない。** 動く被写体を用意して測り直すこと。
API の使い方の問題か、Gen3.1/CCam5 に対する OpenEB 側の制約か、CenturyArks プラグインとの
差分かは、母艦との A/B 比較で切り分ける必要がある。
...
- **ROI が効かない件の切り分け（最優先）。** 動く被写体を用意して測り直し、それでも駄目なら
  母艦（CenturyArks プラグイン経由）で同じコードを実行して比較する。
  Gen3.1 で使える制御は `I_LL_Biases` と `I_ROI` だけなので（③ 参照）、
  ROI が使えないとデータ量を削る手段がバイアスだけになる
- **母艦と同一シーンで比較。** バイアス既定値やイベントレートが一致するか。
  センサ初期化シーケンス（ISSD）に独自調整があればここに差が出る
- `latency_probe` / `motion_probe` を Pi で回し、配送グリッド 4.00 ms 等が再現するか
- trigger in / out の電気的確認（Step 2 の LED 配線と同時にできる）
- 高イベントレート時の挙動
